...
router.get('/:songId', checkJwt, controller.getOne);

router.post('/', checkJwt, controller.create);

// should we do put or patch? i guess both?
router.patch('/:songId', checkJwt, controller.update);
router.put('/:songId', checkJwt, controller.update);

// delete may end up not deleting, but disabling somehow. but for now, here we go
router.delete('/:songId', checkJwt, controller.delete);
